...
class Returner:
    ...
    def finder(self):
        ...
        while True:
            ...
            if ' '.join(sys.argv[1:]) in ('forecast', 'average', 'weekly -n'): 
    ...
    while True:
        if os.path.getsize('weekly.json') == 0 or ' '.join(sys.argv[1:]) == 'weekly -n':
            ...
            try:
                ...
                print("Check your connection sir!")
                break
        elif os.path.getsize('weekly.json') != 0:
            ...
            for g in range(len(WEEK)):
                ...
                try:
                    if reader[WEEK[g]]['today'] == True:
                        is_today = '<- today'
                except KeyError: is_today = ''
            ...
            print(f'{'-'*24}\n{'Highest-Lowest (average)'}\n{'-'*24}')
            print(f'{round(total1/divider, 1)}°C | {round(total2/divider, 1)}°C')
            break
...
#add user changing the name of the weekly if they want so-h
